...
import { ShieldCheck, ArrowRight, FileText, Sparkles } from 'lucide-react';
import { AdPlacementZone } from '@/components/AdPlacementZone';
import { SchemaInjector } from '@/components/SchemaInjector';
import { CategoryHeroIllustration } from '@/components/visuals';

export default function BillsCategoryPage() {
  const { t } = useLanguage();
  const category = CATEGORIES.find((c) => c.id === 'bills') || CATEGORIES[0];
  const articles = Object.values(ARTICLES).filter((a) => a.categoryId === 'bills');
  ...
  return (
    <div className="space-y-10 md:space-y-16 animate-fadeIn">
      ...
      <header className="rounded-3xl doc-card border-2 border-doc-brass/40 p-8 md:p-10 relative overflow-hidden bg-doc-ink text-white shadow-2xl">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          <div className="lg:col-span-4 hidden lg:flex justify-center">
            <CategoryHeroIllustration category="bills" />
          </div>
        </div>
      </header>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-2xl font-serif font-extrabold text-doc-ink dark:text-white">
              {t('Utility Bills Verified Records (2026)', 'یوٹیلیٹی بلز کی تصدیق شدہ فائلز')}
            </h2>
          </div>
      ...
      {/* Ad Zone */}
      <AdPlacementZone slotId="bills-category-bottom" format="horizontal" />
    </div>
  );
}
