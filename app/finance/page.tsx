...
import { ShieldCheck, FileText, ChevronRight, CheckCircle2, ArrowRight, Landmark } from 'lucide-react';
import Link from 'next/link';
import type { Metadata } from 'next';
import { CategoryHeroIllustration } from '@/components/visuals';

export const metadata: Metadata = {
  title: 'Banking & Financial Services | Pakistan Info Hub',
  description:
    'State Bank of Pakistan (SBP) verified guidelines for online bank account opening, Asaan Digital Accounts, biometric video KYC, ISO 13616 IBAN validation, and digital payments.',
  openGraph: {
    title: 'Banking & Financial Services | Pakistan Info Hub',
    description:
      'State Bank of Pakistan (SBP) verified guidelines for online bank account opening, Asaan Digital Accounts, biometric video KYC, ISO 13616 IBAN validation, and digital payments.',
    images: [{ url: 'https://www.pakistaninfohub.com/og-default.jpg', width: 1200, height: 630 }],
    url: 'https://www.pakistaninfohub.com/finance',
  },
  alternates: {
    canonical: 'https://www.pakistaninfohub.com/finance',
  },
};

export default function Page() {
  const category = CATEGORIES.find((c) => c.slug === 'finance') || CATEGORIES[0];
  const articles = Object.values(ARTICLES).filter((a) => a.categoryId === 'finance');
    ...
    <>
      <SchemaInjector
        customSchema={{
          ...
          '@type': 'CollectionPage',
          name: 'Banking & Financial Services | Pakistan Info Hub',
          url: 'https://www.pakistaninfohub.com/finance',
          description:
            'State Bank of Pakistan (SBP) verified guidelines for online bank account opening, Asaan Digital Accounts, biometric video KYC, ISO 13616 IBAN validation, and digital payments.',
        }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
            <div className="md:col-span-8 space-y-4">
              <div className="flex items-center gap-3">
                <VerifiedBadge variant="header" textEn="VERIFIED BANKING REPOSITORY" />
                <span className="font-mono text-xs text-doc-seal uppercase tracking-widest font-bold">
                  {articles.length} OFFICIAL GUIDES &amp; TOOLS
                </span>
              </div>
            ...
            <div className="md:col-span-4 flex justify-center">
              <CategoryHeroIllustration category="finance" />
            </div>
          </div>
        </header>
          ...
          <section className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-doc-dark-card border border-doc-brass/30 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-mono font-bold uppercase tracking-wider text-slate-700 dark:text-doc-brass flex items-center gap-1.5">
                ...
                <span>Featured Banking Utilities &amp; Popular Guides</span>
              </h2>
            </div>
          ...
          {articles.map((art) => (
            ...
            >
              <div className="space-y-2">
                ...
                <h3 className="font-serif font-bold text-base sm:text-lg text-doc-ink dark:text-slate-100 group-hover:text-doc-seal transition leading-snug">
                  {art.titleEn}
                </h3>
              ...
              <div className="pt-4 mt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                <span className="text-slate-500 font-mono">
                  {art.feeStructure && art.feeStructure[0] ? art.feeStructure[0].normal : 'Official Guide'}
                </span>
                <span className="font-serif font-bold text-doc-seal group-hover:translate-x-0.5 transition flex items-center gap-0.5">
