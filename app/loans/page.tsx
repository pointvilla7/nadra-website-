...
export default function LoansCategoryPage() {
  const { t } = useLanguage();
  const category = CATEGORIES.find((c) => c.id === 'loans') || CATEGORIES[0];
  const articles = Object.values(ARTICLES).filter((a) => a.categoryId === 'loans');
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-2xl font-serif font-extrabold text-doc-ink dark:text-white">
              {t('Govt Loans Verified Records (2026)', 'سرکاری قرضہ سکیمیں کی تصدیق شدہ فائلز')}
            </h2>
          </div>
