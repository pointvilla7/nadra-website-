...
import { ShieldCheck, ArrowRight, HeartHandshake, Sparkles } from 'lucide-react';
...
export default function WelfareCategoryPage() {
  const { t } = useLanguage();
  const category = CATEGORIES.find((c) => c.id === 'welfare') || CATEGORIES[0];
  const articles = Object.values(ARTICLES).filter((a) => a.categoryId === 'welfare');
  ...
  return (
    <div className="space-y-10 md:space-y-16 animate-fadeIn">
      ...
      <header className="rounded-3xl doc-card border-2 border-doc-brass/40 p-6 sm:p-8 md:p-10 relative overflow-hidden bg-doc-ink text-white shadow-2xl">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          <div className="lg:col-span-4 flex justify-center">
            <CategoryHeroIllustration category="welfare" />
          </div>
        </div>
      </header>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-2xl font-serif font-extrabold text-doc-ink dark:text-white">
              {t('Social Welfare & Relief Verified Records (2026)', 'بے نظیر و احساس پروگرام کی تصدیق شدہ فائلز')}
            </h2>
          </div>
      ...
      {/* Ad Zone */}
      <AdPlacementZone slotId="welfare-category-bottom" format="horizontal" />
    </div>
  );
}
